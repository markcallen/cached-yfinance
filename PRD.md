...
- **CYF-RETENTION-1:** S3 option-history retention preserves its strict cutoff semantics while deleting expired objects in bounded S3 batch requests, so a first cleanup of historical cache data does not serialize one network request per object.
