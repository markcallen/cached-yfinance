...
class TestCacheKey:
    ...
    def test_cache_key_from_timestamp_datetime(self):
    ...
    def test_cache_key_from_timestamp_pandas(self):
    ...
    def test_cache_key_from_timestamp_with_timezone(self):
...
class TestOptionCacheKey:
    ...
    def test_option_cache_key_creation(self):
    ...
    def test_option_cache_key_with_timestamp(self):
    ...
    def test_option_cache_key_for_calls(self):
    ...
    def test_option_cache_key_for_puts(self):
    ...
    def test_option_cache_key_for_underlying(self):
...
class TestUtilityFunctions:
    ...
    def test_ensure_datetime_pandas(self):
    ...
    def test_ensure_datetime_datetime(self):
...
class TestFileSystemCache:
    ...
    def test_base_dir_path_sanitized_symbol(self, cache):
    ...
    def test_has_cache_miss(self, cache):
        """Test has() method with cache miss."""
        key = CacheKey(symbol="AAPL", interval="1d", day=date(2023, 1, 1))
        assert not cache.has(key)
    
    def test_load_cache_miss(self, cache):
        """Test load() method with cache miss."""
        key = CacheKey(symbol="AAPL", interval="1d", day=date(2023, 1, 1))
        result = cache.load(key)
        assert result is None
    
    def test_store_and_load_dataframe(self, cache, sample_dataframe):
    ...
    def test_store_creates_metadata(self, cache, sample_dataframe):
    ...
    def test_iter_cached_days_empty(self, cache):
        """Test iter_cached_days with no cached data."""
        days = list(cache.iter_cached_days("AAPL", "1d"))
        assert days == []
    
    def test_iter_cached_days_with_data(self, cache, sample_dataframe):
...
class TestFileSystemCacheOptions:
    ...
    def test_option_base_dir_current(self, cache):
    ...
    def test_option_base_dir_historical(self, cache):
        """Test _option_base_dir for historical data."""
        timestamp = "2023-01-01T15:30:00"
        base_dir = cache._option_base_dir("AAPL", "2023-01-20", timestamp)
        expected = cache.root / "AAPL" / "options" / "2023-01-20" / "historical" / "2023-01-01"
        assert base_dir == expected
    
    def test_option_data_path_current(self, cache):
    ...
    def test_option_data_path_historical(self, cache):
        """Test _option_data_path for historical data."""
        timestamp = "2023-01-01T15:30:00"
        path = cache._option_data_path("AAPL", "2023-01-20", "calls", timestamp)
        expected = cache.root / "AAPL" / "options" / "2023-01-20" / "historical" / "2023-01-01" / "calls_153000.parquet"
        assert path == expected
    
    def test_option_meta_path_current(self, cache):
    ...
    def test_option_meta_path_historical(self, cache):
        """Test _option_meta_path for historical data."""
        timestamp = "2023-01-01T15:30:00"
        path = cache._option_meta_path("AAPL", "2023-01-20", timestamp)
        expected = cache.root / "AAPL" / "options" / "2023-01-20" / "historical" / "2023-01-01" / "metadata_153000.json"
        assert path == expected
    
    def test_has_option_chain_calls_miss(self, cache):
        """Test has_option_chain for calls with cache miss."""
        key = OptionCacheKey.for_calls("AAPL", "2023-01-20")
        assert not cache.has_option_chain(key)
    
    def test_has_option_chain_puts_miss(self, cache):
        """Test has_option_chain for puts with cache miss."""
        key = OptionCacheKey.for_puts("AAPL", "2023-01-20")
        assert not cache.has_option_chain(key)
    
    def test_has_option_chain_underlying_miss(self, cache):
        """Test has_option_chain for underlying with cache miss."""
        key = OptionCacheKey.for_underlying("AAPL", "2023-01-20")
        assert not cache.has_option_chain(key)
    
    def test_load_option_chain_calls_miss(self, cache):
        """Test load_option_chain for calls with cache miss."""
        key = OptionCacheKey.for_calls("AAPL", "2023-01-20")
        result = cache.load_option_chain(key)
        assert result is None
    
    def test_load_option_chain_puts_miss(self, cache):
        """Test load_option_chain for puts with cache miss."""
        key = OptionCacheKey.for_puts("AAPL", "2023-01-20")
        result = cache.load_option_chain(key)
        assert result is None
    
    def test_load_option_chain_underlying_miss(self, cache):
        """Test load_option_chain for underlying with cache miss."""
        key = OptionCacheKey.for_underlying("AAPL", "2023-01-20")
        result = cache.load_option_chain(key)
        assert result is None
    
    def test_store_and_load_option_chain(self, cache, sample_option_dataframe, sample_underlying_data):
    ...
    def test_store_option_chain_with_timestamp(self, cache, sample_option_dataframe, sample_underlying_data):
    ...
    def test_store_option_chain_empty_dataframes(self, cache, sample_underlying_data):
    ...
    def test_iter_cached_option_timestamps_with_data(self, cache, sample_option_dataframe, sample_underlying_data):
        ...
        for ts in timestamps:
            cache.store_option_chain(symbol, expiration, sample_option_dataframe, sample_option_dataframe, sample_underlying_data, ts)
        
        cached_timestamps = list(cache.iter_cached_option_timestamps(symbol, expiration))
