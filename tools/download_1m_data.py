...
def download_1m_data(ticker: str, days: int = 60, cache_dir: str = None) -> None:
    ...
    if days > max_days_for_1m:
        print(
            f"⚠️  Warning: Yahoo Finance limits 1-minute data to ~30 days. Adjusting from {days} to {max_days_for_1m} days."
        )
    ...
    try:
        ...
        data = client.download(
            ticker.upper(), start=start_date, end=end_date, interval="1m", progress=True
        )
...
Examples:
  ...
  %(prog)s TSLA --cache-dir ~/my_cache  # Use custom cache directory
    ...
    if args.days > 30:
        print(
            "⚠️  Warning: Yahoo Finance limits 1-minute data to ~30 days. Data may be incomplete for older dates."
        )
