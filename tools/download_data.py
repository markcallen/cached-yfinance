...
def download_data(ticker: str, interval: str = "1d", days: int = 60, cache_dir: str = None) -> None:
    """
    ...
    if is_intraday and days > 60:
        raise ValueError(f"❌ Error: Days cannot exceed 60 for intraday intervals. Got {days} days for interval '{interval}'.")
    ...
    if is_intraday and days > 30:
        ...
        print(f"⚠️  Warning: Yahoo Finance limits intraday data to ~30 days. Adjusting from {days} to {max_days_for_intraday} days.")
...
Examples:
  ...
  %(prog)s GOOGL --cache-dir ~/my_cache          # Use custom cache directory
    ...
    if args.interval not in valid_intervals:
        print(f"❌ Error: Invalid interval '{args.interval}'. Valid intervals: {', '.join(valid_intervals)}")
        sys.exit(1)
    ...
    if is_intraday and args.days > 60:
        print(f"❌ Error: Days cannot exceed 60 for intraday intervals. Got {args.days} days for interval '{args.interval}'.")
        sys.exit(1)
    
    if is_intraday and args.days > 30:
        print("⚠️  Warning: Yahoo Finance limits intraday data to ~30 days. Data may be incomplete for older dates.")
