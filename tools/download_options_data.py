...
def download_option_chain(ticker: str, expiration: Optional[str] = None, cache_dir: str = None) -> None:
    """
    ...
    try:
        ...
        if not option_chain.calls.empty:
            ...
            if not active_calls.empty:
                ...
                top_calls = active_calls.nlargest(5, 'volume')[['strike', 'lastPrice', 'bid', 'ask', 'volume', 'openInterest', 'impliedVolatility']]
        ...
        if not option_chain.puts.empty:
            ...
            if not active_puts.empty:
                ...
                top_puts = active_puts.nlargest(5, 'volume')[['strike', 'lastPrice', 'bid', 'ask', 'volume', 'openInterest', 'impliedVolatility']]
                print(top_puts.round(4))
        
        # At-the-money analysis
        if underlying_price and not option_chain.calls.empty and not option_chain.puts.empty:
            ...
            if not calls_df.empty:
                ...
                print(f"   ATM Call (${atm_call['strike']:.2f}): ${atm_call['lastPrice']:.2f}, IV: {atm_call['impliedVolatility']:.4f}")
            
            if not puts_df.empty:
                atm_put = puts_df.loc[puts_df['distance_from_atm'].idxmin()]
                print(f"   ATM Put (${atm_put['strike']:.2f}): ${atm_put['lastPrice']:.2f}, IV: {atm_put['impliedVolatility']:.4f}")
...
def download_all_expirations(ticker: str, cache_dir: str = None) -> None:
    ...
    try:
        ...
        print(f"📅 Found {len(expirations)} expiration dates: {expirations[:5]}{'...' if len(expirations) > 5 else ''}")
...
All downloads automatically include timestamps for historical data tracking.
    ...
    parser.add_argument(
        "ticker",
        help="Stock ticker symbol (e.g., AAPL, TSLA, IWM)"
    )
