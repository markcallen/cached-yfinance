...
def collect_options_data(ticker: str, client: cyf.CachedYFClient, max_expirations: int, logger: logging.Logger) -> Dict:
    """
    ...
    try:
        ...
        for expiration in expirations_to_process:
            try:
                ...
                calls_count = len(option_chain.calls) if not option_chain.calls.empty else 0
                puts_count = len(option_chain.puts) if not option_chain.puts.empty else 0
                contracts = calls_count + puts_count
                
                if contracts > 0:
                    total_contracts += contracts
                    logger.debug(f"{ticker} {expiration}: {calls_count} calls, {puts_count} puts")
        ...
        logger.info(f"{ticker}: Successfully collected {total_contracts:,} contracts across {len(expirations_to_process)} expirations")
...
*/15 9-16 * * 1-5 /usr/bin/python3 /path/to/market_hours_collector.py --config /path/to/config.json
    ...
    parser.add_argument(
        "--cache-dir",
        help="Custom cache directory path (overrides config file)"
    )
