...
def basic_option_chain_example():
    ...
    print(f"Underlying price: ${option_chain.underlying.get('regularMarketPrice', 'N/A')}")
    
    if not option_chain.calls.empty:
        print("\nSample call options:")
        print(option_chain.calls[['strike', 'lastPrice', 'bid', 'ask', 'volume', 'openInterest']].head())
    
    if not option_chain.puts.empty:
        print("\nSample put options:")
        print(option_chain.puts[['strike', 'lastPrice', 'bid', 'ask', 'volume', 'openInterest']].head())
...
def option_greeks_analysis():
    ...
    print(top_volume[['strike', 'lastPrice', 'volume', 'openInterest', 'impliedVolatility']])
    
    print("\nTop 5 by open interest:")
    top_oi = active_calls.nlargest(5, 'openInterest')
    print(top_oi[['strike', 'lastPrice', 'volume', 'openInterest', 'impliedVolatility']])
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("Make sure you have an internet connection and the required dependencies installed.")
