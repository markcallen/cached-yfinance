...
def analyze_historical_options(ticker: str = "AAPL", expiration: str = None):
    ...
    if not timestamps:
        ...
        print("   Try running the market_hours_collector.py first to collect historical data")
        return
    ...
    for timestamp in timestamps[:10]:  # Limit to first 10 for demo
        try:
            option_chain = client.get_option_chain(ticker, expiration, timestamp=timestamp)
            ...
            calls_volume = option_chain.calls['volume'].sum() if not option_chain.calls.empty else 0
            puts_volume = option_chain.puts['volume'].sum() if not option_chain.puts.empty else 0
    ...
    for _, row in df.tail(5).iterrows():
        ...
        print(f"{time_str}: ${row['underlying_price']:.2f}, Vol: {row['total_volume']:,.0f}, P/C: {row['put_call_ratio']:.2f}")
...
def compare_current_vs_historical(ticker: str = "AAPL"):
    ...
    current_volume = (current_chain.calls['volume'].sum() + 
    ...
    expirations = client.get_options_expirations(ticker)
    if expirations:
        timestamps = list(client.cache.iter_cached_option_timestamps(ticker, expirations[0]))
        if timestamps:
...
if __name__ == "__main__":
    ...
    print("2. All downloads now automatically include timestamps for historical tracking")
